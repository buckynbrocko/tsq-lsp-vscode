...
export class NamedNode extends SingularMatchLint {
    static lintName = 'lint.named-node' as const;
...
function lintSubnode(subnode: TSNode, environment: TypeEnvironment, checkable?: CheckableNamed): LintResult {
    switch (subnode.type) {
        case 'named_node':
        case 'anonymous_node':
            return !!checkable ? lintChild(subnode, checkable) : [];
        case 'field_definition':
...
function lintChild(subnode: TSNode, checkable: CheckableNamed): LintResult {
    ...
    const name: string = TypeName.fromNode(subnode, identifier) ?? '';
